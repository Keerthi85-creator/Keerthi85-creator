# 👋 Hi, I’m Keerthi M
**PORTFOLIO:** [portfolio](https://keerthi85-creator.github.io/keerthi.github.io/)
**LinkedIn:** [Connect with me](https://www.linkedin.com/in/keerthi-mubarakpur/)    
**🎓 B.Tech Student at IIT Dharwad**  
**💡 Passionate about building intelligent AI-powered applications**

---

## 🔍 What I’m Into
- **Artificial Intelligence (AI)** and **Machine Learning (ML)** applications  
- **Generative AI**, **Large Language Models (LLMs)**, and building **agentic AI tools**  
- Open-source contributions and **collaborative AI projects**

---

## 🚀 What I'm Learning
- Advanced **AI/ML algorithms** and frameworks  
- **Cloud-based AI workflows** (e.g., Amazon Bedrock Agents + Lambda)  
- Designing **intelligent agent systems** and AI-powered solutions

---

## 🤝 Let’s Collaborate On
- AI/ML projects and **intelligent applications**  
- LLM & AI agent **usability testing**  
- Student-led AI initiatives and **open-source AI tools**

---

## 📫 Reach Me
- **Email:** keerthi.mubarakpur@gmail.com  
- **LinkedIn:** [Connect with me](https://www.linkedin.com)

---

## 📂 My Recent Project
**AI Business Assistant** using Amazon Bedrock (MCP Project)

---

## ⚡ Fun Fact
I love blending **creativity with AI** — whether it's designing smart agents that understand human prompts or experimenting with generative AI ideas.
